...
function courtList(){
	var columns = 6;
	var courts = $(".courts");
	var list = $(".courts-list li");
	var count = list.length;
	var j = 0;

	$.each(list, function(i,v){
		if(i % Math.ceil(count/6) == 0){ 
			j++;
		}
		$(this).addClass("courtListitem")
			   .addClass("group"+j);
	})	
	//$(".courts_list").unwrap();
	//$(".courtListitem").unwrap();
	for(n = j; j > 0; j--){
		//$(".group"+j).wrapAll($("<li class='sub-nav-group'><ol class='courts'>"));
		$(".group"+j).wrapAll($("<li class='courts-group'><ol class='county-list'>"));
	}
	$(".county-list").each(function(){
		var first = $(this).find("a").first().text();
		var last = $(this).find("a").last().text();
		$("<h4 class='sr-only'>").text(first + " through " + last).insertBefore($(this));
	});
}
function setMenuSize(){
	$(".nav-item").each(function(){
		$(this).mouseenter(function(){
			var x = $(this).find(".sub-nav").height();
			$(this).find(".sub-nav-group").first().height(x);
